<template>
  <article class="flex items-start space-x-6 p-6 mx-auto w-full xl:w-1/3">
    <img
      :src="image"
      alt=""
      width="60"
    ...
    <div class="min-w-0 relative flex-auto">
      <h2 class="font-semibold text-gray-900 truncate pr-20">
        {{ title }}
      </h2>
      <dl class="mt-2 flex flex-wrap text-sm leading-6 font-medium">
        <div class="absolute top-0 right-0 flex items-center space-x-1">
          <dt class="text-sky-500 flex">
        ...
        <div>
          <dd class="flex items-center">{{}}</dd>
        </div>
        <div>
          <dd class="flex items-center">{{}}</dd>
        </div>
        <div class="flex-none w-full mt-2 font-normal">
          <dd class="text-gray-400">{{ calories }} Calories</dd>
        </div>
      </dl>
    </div>
  </article>
...
export default {
  props: {
    ...
    description: {
      ...
      required: false,
      default () {
        return 'This product has no description'
      }
    },
    calories: {
      type: String,
